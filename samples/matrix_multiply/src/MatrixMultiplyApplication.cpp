...
bool MatrixMultiplyApplication::ParseCommandLineArguments(int argc, char **argv)
{
	...
	{
		for (const auto &rArg : {Arg{"--count", &mCount},
								 Arg{"--M", &mM},
								 Arg{"--N", &mN},
								 Arg{"--K", &mK}})
		{
...
{
	auto *pShader = GraphicsSystem::GetInstance()->FindShader(
		"ScalarLoop");
	...
	struct
	{
		...
		(unsigned int)mM, (unsigned int)mN, (unsigned int)mK};
	...
	for (int i = 0; i < mCount; ++i)
	{
		mpGraphicsContext->PushDebugMarker(("C_" + std::to_string(i) + " = A_" + std::to_string(i) + " * B_" + std::to_string(i)).c_str());
		{
...
void MatrixMultiplyApplication::ProcessResults()
{
	...
	auto AreEqual = [](float a, float b, float epsilon = 1e-3)
	{
	...
	for (size_t i = 0; i < mCount; ++i)
	{
		for (size_t m = 0, c = 0; m < mM; ++m)
		{
			for (size_t n = 0; n < mN; ++n, ++c)
			{
				...
				{
					std::cout << " C_" << i << "[" << m << ", " << n << "] GPU/GPU values differ too much (CPU: " << mC_values_cpu[i][c] << " / GPU: " << C_values_gpu[i][c] << ")" << std::endl;
					diff = true;
				}
			}
		}
