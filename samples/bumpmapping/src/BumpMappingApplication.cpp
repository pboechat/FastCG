...
void BumpMappingApplication::OnStart()
{
	...
	mLightPtr->SetDiffuseColor(glm::vec4(0.8f, 0.8f, 0.8f, 1.0f));
	mLightPtr->SetSpecularColor(glm::vec4(0.6f, 0.6f, 0.6f, 1.0f));
	...
	mSphereColorMapTexturePtrs.push_back(LoadPNGAsTexture("textures/SphereColorMap2.png"));
	mSphereBumpMapTexturePtrs.push_back(LoadPNGAsTexture("textures/SphereColorMap2.png"));

	MaterialPtr floorMaterialPtr;
#ifdef USE_OPENGL4
	floorMaterialPtr = new Material(mBumpedSpecularShaderPtr);
	floorMaterialPtr->SetTexture("colorMap", mFloorColorMapTexturePtrs[mCurrentFloorTextureIndex]);
	floorMaterialPtr->SetTexture("bumpMap", mFloorBumpMapTexturePtrs[mCurrentFloorTextureIndex]);
	floorMaterialPtr->SetFloat("shininess", 5.0f);
	floorMaterialPtr->SetTextureTiling("colorMap", mFloorTextureTiling);
	floorMaterialPtr->SetTextureTiling("bumpMap", mFloorTextureTiling);

	mFloorMaterialPtr = floorMaterialPtr;
#else
	floorMaterialPtr = new Material();
	floorMaterialPtr->SetTexture(mFloorColorMapTexturePtr);
#endif

	GeometryPtr floorPtr = StandardGeometries::CreateXZPlane((float)FIELD_SIZE, (float)FIELD_SIZE, 1, 1, glm::vec3(0, 0, 0), floorMaterialPtr);
#ifdef USE_OPENGL4
	...
	for (unsigned int z = 0; z < FIELD_SIZE; z += 10)
	{
		for (unsigned int x = 0; x < FIELD_SIZE; x += 10)
		{
			...
			sphereMaterialPtr->SetTexture(mSphereColorMapTexturePtr);
#endif
