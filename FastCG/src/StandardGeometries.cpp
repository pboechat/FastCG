...
namespace FastCG
{
	std::unique_ptr<Mesh> StandardGeometries::CreateXYPlane(float width, float height, uint32_t xSegments, uint32_t ySegments, const glm::vec3& rCenter)
	{
		...
		auto* pMesh = new Mesh(vertices, normals, uvs, indexes);
		pMesh->CalculateTangents();
		return std::unique_ptr<Mesh>(pMesh);
	}

	std::unique_ptr<Mesh> StandardGeometries::CreateXZPlane(float width, float depth, uint32_t xSegments, uint32_t zSegments, const glm::vec3& rCenter)
	{
		std::vector<glm::vec3> vertices;
		std::vector<glm::vec3> normals;
		std::vector<glm::vec2> uvs;
	...
	std::unique_ptr<Mesh> StandardGeometries::CreateSphere(float radius, uint32_t slices)
	{
		std::vector<glm::vec3> vertices;
		std::vector<glm::vec3> normals;
		std::vector<glm::vec2> uvs;
		...
		auto* pMesh = new Mesh(vertices, normals, uvs, indices);
		pMesh->CalculateTangents();
		return std::unique_ptr<Mesh>(pMesh);
	}
