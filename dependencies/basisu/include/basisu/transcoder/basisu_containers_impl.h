...
#define HASHMAP_TEST_VERIFY(c)                         \
   do                                                  \
   {                                                   \
      if (!(c))                                        \
         handle_hashmap_test_verify_failure(__LINE__); \
   } while (0)
   ...
   {
   public:
      counted_obj(uint32_t v = 0) : m_val(v)
      {
         m_count++;
      }

      counted_obj(const counted_obj &obj) : m_val(obj.m_val)
      {
         m_count++;
      }
