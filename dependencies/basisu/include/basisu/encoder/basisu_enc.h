...
namespace basisu
{
	...
	public:
		...
		inline vec(const vec<OtherN, OtherT> &other) { set(other); }
		...
		template <uint32_t OtherN, typename OtherT>
		inline vec &operator=(const vec<OtherN, OtherT> &rhs)
		{
			set(rhs);
			return *this;
		}
	...
	template <typename T>
	inline void indirect_sort(uint32_t num_indices, uint32_t *pIndices, const T *pKeys)
	{
...
#undef BASISU_GET_KEY
	...
	public:
		...
		inline color_rgba(const basist::color32 &other) : r(other.r),
														  g(other.g),
														  b(other.b),
														  a(other.a)
		{
	...
	public:
		...
		tree_vector_quant() : m_next_codebook_index(0)
		{
		}
	...
	private:
		...
		public:
			...
			// vecs is erased
			inline void set(const TrainingVectorType &org, uint64_t weight, float var, basisu::vector<uint32_t> &vecs)
			{
				m_origin = org;
				m_weight = weight;
				m_var = var;
	...
	public:
		...
		image(uint32_t w, uint32_t h, uint32_t p = UINT32_MAX) : m_width(0), m_height(0), m_pitch(0)
		{
			resize(w, h, p);
		}

		image(const uint8_t *pImage, uint32_t width, uint32_t height, uint32_t comps) : m_width(0), m_height(0), m_pitch(0)
		{
			init(pImage, width, height, comps);
		}

		image(const image &other) : m_width(0), m_height(0), m_pitch(0)
		{
			*this = other;
		}
	...
	{
	public:
		imagef() : m_width(0), m_height(0), m_pitch(0)
		{
		}

		imagef(uint32_t w, uint32_t h, uint32_t p = UINT32_MAX) : m_width(0), m_height(0), m_pitch(0)
		{
			resize(w, h, p);
		}

		imagef(const imagef &other) : m_width(0), m_height(0), m_pitch(0)
		{
			*this = other;
		}
	...
	public:
		...
		vector2D(uint32_t w, uint32_t h) : m_width(0),
										   m_height(0)
		{
			resize(w, h);
		}
