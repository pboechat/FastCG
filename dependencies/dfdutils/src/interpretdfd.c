                                     ...
                                     uint32_t *wordBytes)
{
    ...
    if ((BDFDB[KHR_DF_WORD_BYTESPLANE0] & ~KHR_DF_MASK_BYTESPLANE0) || BDFDB[KHR_DF_WORD_BYTESPLANE4])
    ...
    for (uint32_t i = 0; i < numSamples; ++i)
    {
        ...
        const bool isNormalized = isFloat ? *(float *)(void *)&BDFDB[KHR_DF_WORD_SAMPLESTART +
    ...
    else if (KHR_DFDVAL(BDFDB, MODEL) == KHR_DF_MODEL_RGBSDA)
    {
        ...
        if (KHR_DFDVAL(BDFDB, TRANSFER) == KHR_DF_TRANSFER_SRGB)
            result |= i_SRGB_FORMAT_BIT;
            ...
            for (uint32_t sampleCounter = 0; sampleCounter < numSamples; ++sampleCounter)
            {
                ...
                if (sampleChannel == currentChannel)
                {
                    ...
                    if (sampleByteOffset == currentByteOffset - 1U &&        /* One byte earlier */
                        ...
                        (sampleBitOffset & 7U) == 0)
                    { /* Start at the beginning of the byte */
        ...
        else
        {
            ...
            for (uint32_t sampleCounter = 0; sampleCounter < numSamples; ++sampleCounter)
            {
                ...
                if (sampleChannel == currentChannel)
                {
                    ...
                    if (sampleByteOffset == currentByteOffset - 1)
                    { /* One byte earlier */
