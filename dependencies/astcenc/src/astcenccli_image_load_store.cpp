	...
	int y_flip)
{
	...
	if (wf)
	{
		void *dataptr = (bitness == 16) ? reinterpret_cast<void *>(row_pointers16[0][0]) : reinterpret_cast<void *>(row_pointers8[0][0]);
	...
	const char *filename)
{
	...
	{
		if (strcmp(eptr, storer_descs[i].ending1) == 0 || strcmp(eptr, storer_descs[i].ending2) == 0)
		{
	...
	unsigned int &component_count)
{
	...
	{
		if (loader_descs[i].ending1 == nullptr || strcmp(eptr, loader_descs[i].ending1) == 0 || strcmp(eptr, loader_descs[i].ending2) == 0)
		{
