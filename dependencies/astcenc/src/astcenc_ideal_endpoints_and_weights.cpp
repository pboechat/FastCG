	...
	vfloat4 rgbo_vectors[BLOCK_MAX_PARTITIONS])
{
	...
	for (unsigned int i = 0; i < partition_count; i++)
	{
		...
		else
		{
			...
			vfloat4 color_mss1 = (left_sum * left_sum) + (2.0f * middle_sum * middle_sum) + (right_sum * right_sum);

			float ls_mss1 = (lmrs_sum.lane<0>() * lmrs_sum.lane<0>()) + (2.0f * lmrs_sum.lane<1>() * lmrs_sum.lane<1>()) + (lmrs_sum.lane<2>() * lmrs_sum.lane<2>());
	...
	int plane2_component)
{
	...
	else
	{
		...
		vfloat4 color_mss1 = (left1_sum * left1_sum) + (2.0f * middle1_sum * middle1_sum) + (right1_sum * right1_sum);
	...
	else
	{
		...
		vfloat4 color_mss2 = (left2_sum * left2_sum) + (2.0f * middle2_sum * middle2_sum) + (right2_sum * right2_sum);
